...
def index():
    ...
    dbconn=current_app.config['DBCONN']
    ...
    if dbconn != None:
        ...
        result="Something wen't wrong while connection to the database ..."
        flash('ERROR: App couldnt connect to the DB ...')
...
def reload():
    ...
    try:
        ...
        orders.load_orders_only(current_app.config['QUERYORDERS'],None,None)
        orders.load_orders_products(current_app.config['QUERYORDERPRODUCTS'],None,None)
        """Udating the order object in config.py, with the most recent data in the database"""
        current_app.config['ORDERS']=orders
